...
const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'ines-live-audit-'));
const results = [];
const ids = {};
const filesToRemove = new Set();

if (!ADMIN_PASSWORD) {
  throw new Error('Set INES_ADMIN_PASSWORD before running the live acceptance suite.');
}
...
const docxPath = path.join(tempDir, 'ines-live-audit.docx');
...
const db = await mysql.createConnection({
  ...
  database: process.env.DB_NAME || 'ines_intelligent_library',
});
...
try {
  ...
  await test('Real student registration', async () => {
    const response = await api('/auth/register', {
      method: 'POST',
      body: {
        full_name: 'INES Live Audit Student',
  ...
  await test('Administrator creates a real lecturer account', async () => {
    const response = await api('/users', {
      ...
      token: admin.token,
      body: {
        full_name: 'INES Live Audit Lecturer',
  ...
  await test('Lecturer creates a course-linked catalog book', async () => {
    const response = await api('/books', {
      ...
      body: {
        title: `INES Live Audit Book ${runId}`,
  ...
  await test('Header/catalog search query logs real results', async () => {
    const response = await api('/search/books?q=INES%20Live%20Audit', { token: student.token });
  ...
  await test('Student private Word upload converts to PDF', async () => {
    const response = await upload(
      '/personal-books',
      student.token,
      { title: 'Private Live Audit Book', author: 'INES Audit' },
      docxPath,
  ...
  await test('Manual microphone transcript reaches voice search and logging', async () => {
    const form = new FormData();
    form.append('transcript', 'INES Live Audit');
    ...
    if (!logs.data.some((row) => row.transcript === 'INES Live Audit')) throw new Error('voice log missing');
