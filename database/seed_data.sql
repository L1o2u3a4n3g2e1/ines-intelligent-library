...
SELECT r.id, 'Student Demo', 'student@ines.ac.rw', '+250700000001', '$2y$12$LbCWrHGIcrjNtq2PwZL8sO3JjBLhNUrO53QXn7OGiFMEAtVV3/20O', NOW(), 'active'
FROM roles r WHERE r.code = 'STUDENT'
ON DUPLICATE KEY UPDATE full_name = VALUES(full_name), role_id = VALUES(role_id);

INSERT INTO users (role_id, full_name, email, phone, password_hash, email_verified_at, status)
SELECT r.id, 'Lecturer Demo', 'lecturer@ines.ac.rw', '+250700000002', '$2y$12$LbCWrHGIcrjNtq2PwZL8sO3JjBLhNUrO53QXn7OGiFMEAtVV3/20O', NOW(), 'active'
...
WHERE u.email = 'student@ines.ac.rw'
...
SELECT a.id, cat.id, f.id, d.id, c.id, 'Digital Library Systems', '978-1-00101-001-1', 'INES Press', 2022,
...
INSERT INTO books (author_id, category_id, faculty_id, department_id, course_id, title, isbn, publisher, publication_year, description, keywords, shelf_location, total_copies, available_copies, status)
SELECT a.id, cat.id, f.id, d.id, c.id, 'Human Computer Interaction', '978-1-00102-002-8', 'INES Press', 2021,
...
VALUES
  ('system_name', 'INES Intelligent Digital Library', 'string', 'System display name', 1),
  ('institution_name', 'INES Ruhengeri', 'string', 'Institution name', 1),
...
WHERE u.email IN ('student@ines.ac.rw', 'lecturer@ines.ac.rw', 'library@gmail.com', 'lecturer@gmail.com')
  AND NOT EXISTS (
    ...
    WHERE n.user_id = u.id
      AND n.type = 'system'
      AND n.title = 'Welcome to INES Library'
      AND n.status = 'active'
  );
