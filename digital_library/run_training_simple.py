"""
LSTM Speech Recognition - Automated Training Pipeline (Windows Compatible)
Simplified startup script that guides through the entire process
"""

import os
import sys
import subprocess
from pathlib import Path
import json
from datetime import datetime


def print_header(title):
    """Print formatted header"""
    print("\n" + "=" * 80)
    print("[*] " + title.upper())
    print("=" * 80 + "\n")


def check_dependencies():
    """Check if all dependencies are installed"""
    print_header("Checking Dependencies")

    required = {
        'torch': 'PyTorch',
        'librosa': 'Librosa',
        'soundfile': 'SoundFile',
        'kagglehub': 'Kaggle Hub',
        'numpy': 'NumPy',
        'scipy': 'SciPy',
        'tqdm': 'TQDM',
        'sklearn': 'Scikit-learn',
        'matplotlib': 'Matplotlib'
    }

    missing = []
    for package, name in required.items():
        try:
            __import__(package)
            print("[OK] " + name)
        except ImportError:
            print("[FAIL] " + name)
            missing.append(package)

    if missing:
        print("\n[!] Missing packages: " + ", ".join(missing))
        print("\nInstalling missing dependencies...\n")
        try:
            subprocess.check_call([
                sys.executable, '-m', 'pip', 'install', '-q'] + missing
            )
            print("\n[OK] Dependencies installed!")
        except subprocess.CalledProcessError:
            print("[FAIL] Failed to install dependencies")
            return False

    return True


def check_directories():
    """Verify required directories exist"""
    print_header("Checking Directory Structure")

    dirs = [
        'digital_library/models/stt',
        'digital_library/data/processed',
        'digital_library/data/kinyarwanda',
    ]

    for dir_path in dirs:
        Path(dir_path).mkdir(parents=True, exist_ok=True)
        print("[OK] " + dir_path)

    return True


def check_kaggle_credentials():
    """Check if Kaggle credentials are set up"""
    print_header("Checking Kaggle Setup")

    kaggle_json = Path.home() / '.kaggle' / 'kaggle.json'

    if kaggle_json.exists():
        print("[OK] Kaggle credentials found!")
        return True
    else:
        print("[!] Kaggle credentials not found")
        print("\n[INFO] To set up Kaggle:")
        print("   1. Visit: https://www.kaggle.com/settings/account")
        print("   2. Click 'Create New Token'")
        print("   3. Save the kaggle.json file")
        print("\n[INFO] Running setup wizard...\n")

        try:
            subprocess.check_call([
                sys.executable, 'digital_library/kaggle_setup.py'
            ])
            return True
        except Exception as e:
            print("[!] Setup wizard: " + str(e))
            print("\n[!] You can set up Kaggle manually later.")
            return False


def download_dataset():
    """Download Kinyarwanda dataset"""
    print_header("Downloading Kinyarwanda Dataset")

    dataset_path = Path('digital_library/data/kinyarwanda')
    audio_files = list(dataset_path.rglob('*.wav')) + list(dataset_path.rglob('*.mp3'))

    if len(audio_files) > 100:
        print("[OK] Dataset already downloaded (" + str(len(audio_files)) + " audio files)")
        return True

    print("[INFO] Downloading dataset from Kaggle...")
    print("[INFO] This may take 5-10 minutes depending on your connection\n")

    try:
        import kagglehub
        path = kagglehub.dataset_download("programmerdatch/kinyarwanda-dataset")
        print("\n[OK] Dataset downloaded to: " + path)

        # Move to our directory structure
        import shutil
        source = Path(path)
        dest = dataset_path

        for item in source.iterdir():
            if item.is_dir():
                shutil.copytree(item, dest / item.name, dirs_exist_ok=True)
            else:
                shutil.copy2(item, dest)

        audio_files = list(dest.rglob('*.wav')) + list(dest.rglob('*.mp3'))
        print("[OK] Found " + str(len(audio_files)) + " audio files")
        return len(audio_files) > 0

    except Exception as e:
        print("[!] Download failed: " + str(e))
        print("\n[!] You can download manually from:")
        print("   https://www.kaggle.com/datasets/programmerdatch/kinyarwanda-dataset")
        return False


def train_model():
    """Train LSTM model"""
    print_header("Training LSTM Model")

    print("[INFO] Starting training...")
    print("[INFO] This will take 30-60 minutes depending on your hardware")
    print("[INFO] (GPU recommended)\n")

    try:
        subprocess.check_call([
            sys.executable, 'digital_library/train_lstm_kinyarwanda.py'
        ])
        return True
    except Exception as e:
        print("[FAIL] Training failed: " + str(e))
        return False


def evaluate_model():
    """Evaluate trained model"""
    print_header("Evaluating Model")

    model_path = Path('digital_library/models/stt/best_kinyarwanda_lstm.pt')

    if not model_path.exists():
        print("[!] Model not found. Train the model first.")
        return False

    print("[INFO] Evaluating model performance...\n")

    try:
        subprocess.check_call([
            sys.executable, 'digital_library/evaluate_lstm_model.py'
        ])
        return True
    except Exception as e:
        print("[!] Evaluation: " + str(e))
        return False


def show_results():
    """Show training results"""
    print_header("Training Summary")

    results_path = Path('digital_library/models/stt/kinyarwanda_training_results.json')

    if results_path.exists():
        try:
            with open(results_path) as f:
                results = json.load(f)

            metrics = results.get('test_metrics', {})

            print("[OK] TRAINING COMPLETE!\n")
            print("[METRICS] Model Performance:")
            print("   Accuracy: " + str(round(metrics.get('accuracy', 0)*100, 2)) + "%")
            print("   WER: " + str(round(metrics.get('wer', 0)*100, 2)) + "%")
            print("   Precision: " + str(round(metrics.get('precision', 0)*100, 2)) + "%")
            print("   Recall: " + str(round(metrics.get('recall', 0)*100, 2)) + "%")
            print("   F1-Score: " + str(round(metrics.get('f1', 0)*100, 2)) + "%")

            print("\n[PATH] Model Location:")
            print("   " + str(results_path.parent / 'best_kinyarwanda_lstm.pt'))

            print("\n[PATH] Full Report:")
            print("   " + str(results_path))

        except Exception as e:
            print("[!] Could not read results: " + str(e))
    else:
        print("[INFO] Training results not available yet")


def main():
    """Main training pipeline"""

    print("\n" + "="*80)
    print("[*] LSTM SPEECH RECOGNITION - AUTOMATED TRAINING PIPELINE")
    print("="*80)

    print("\n[INFO] This script will:")
    print("   1. Check dependencies")
    print("   2. Verify directory structure")
    print("   3. Setup Kaggle credentials")
    print("   4. Download Kinyarwanda dataset")
    print("   5. Train LSTM model (30-60 min)")
    print("   6. Evaluate performance")

    proceed = input("\n[?] Ready to start? (y/n): ").lower().strip()
    if proceed != 'y':
        print("[INFO] Canceled")
        return

    start_time = datetime.now()

    # Run steps
    steps = [
        ("Dependencies", check_dependencies),
        ("Directories", check_directories),
        ("Kaggle", check_kaggle_credentials),
        ("Dataset", download_dataset),
        ("Training", train_model),
        ("Evaluation", evaluate_model),
    ]

    completed = []
    for step_name, step_func in steps:
        try:
            if step_func():
                completed.append(step_name)
                print("\n[OK] " + step_name + " completed")
            else:
                print("\n[!] " + step_name + " skipped or failed")
                # Ask to continue
                cont = input("\n[?] Continue anyway? (y/n): ").lower().strip()
                if cont != 'y':
                    break
        except KeyboardInterrupt:
            print("\n\n[!] Interrupted by user")
            break
        except Exception as e:
            print("\n[FAIL] " + step_name + " error: " + str(e))
            cont = input("\n[?] Continue anyway? (y/n): ").lower().strip()
            if cont != 'y':
                break

    # Show results
    show_results()

    # Summary
    elapsed = datetime.now() - start_time
    print("\n[TIME] Total time: " + str(elapsed))
    print("[OK] Completed steps: " + ", ".join(completed))

    print("\n" + "=" * 80)
    print("[SUCCESS] LSTM TRAINING PIPELINE COMPLETE!")
    print("=" * 80)

    print("\n[NEXT] Steps:")
    print("   1. Review results in models/stt/kinyarwanda_training_results.json")
    print("   2. Test on microphone: python digital_library/test_lstm_inference.py")
    print("   3. Integrate with API: voice_recognition_service_enhanced.py")
    print("   4. See LSTM_SPEECH_RECOGNITION.md for documentation")


if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
        print("\n\n[INFO] Interrupted")
        sys.exit(0)
